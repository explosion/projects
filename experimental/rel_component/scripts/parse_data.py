...
ALL_LABELS = ["Pos-Reg", "Neg-Reg", "Reg", "Binds", "No-Effect"]
SYMM_LABELS = ["Binds"]
        ...
        for line in jsonfile:
            ...
            if example["answer"] == "accept":
                try:
                    ...
                    article_id.replace("BioNLP 2011 Genia Shared Task, ", "")
                    ...
                    msg.fail(f"Skipping example: {example}")

    docbin = DocBin(docs=train_docs, store_user_data=True)
    docbin.to_disk(train_file)
    msg.info(f"Wrote {len(train_docs)} training sentences from {len(train_ids)} articles to {train_file}")

    docbin = DocBin(docs=dev_docs, store_user_data=True)
    docbin.to_disk(dev_file)
    msg.info(f"Wrote {len(dev_docs)} dev sentences from {len(dev_ids)} articles to {dev_file}")

    docbin = DocBin(docs=test_docs, store_user_data=True)
    docbin.to_disk(test_file)
    msg.info(f"Wrote {len(test_docs)} test sentences from {len(test_ids)} articles to {test_file}")
