...
class CandidateSelector(NearestNeighborCandidateSelector):
    ...
    def _fetch_candidates(
        self, dataset_id: str, span: Span, kb: KnowledgeBase, max_n_candidates: int, lexical_similarity_cutoff: float = .5
    ) -> Iterable[int]:
        ...
        nn_idx = self._lookup_structs[dataset_id].kneighbors(target_vec.reshape((1, -1)))[1][0]
        ...
        return {
            cand
            for cands_for_alias in [kb.get_alias_candidates("_" + cei + "_") for cei in candidate_entity_ids]
