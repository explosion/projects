...
class CandidateSelector(NearestNeighborCandidateSelector):
    ...
    def _fetch_candidates(
        self, dataset_id: str, span: Span, kb: KnowledgeBase, max_n_candidates: int, similarity_cutoff: float = .5
    ) -> Iterable[int]:
        all_cands = [
            kb.get_alias_candidates(entry[1]) for entry in self._lookup_structs[dataset_id].get(span.text, [])[:max_n_candidates]
        ]
